/**
 * @file aac_parser_sample.c
 * @brief AAC 码流解析示例程序
 * 
 * 演示如何使用 aac_parser 解析码流文件并通过 ringbuf 进行数据传输
 * 
 * 功能：
 * 1. 一个线程：使用 aac_parser 解析码流文件，将帧推送到 ringbuf
...
#include <pthread.h>
#include <sys/prctl.h>
#include "../aac_parser/aac_parser.h"
#include "../ringbuf/ringbuf.h"
#include "../log.h"

// 全局变量
static RingBuffer_t *g_ringbuf = NULL;
static AACParser_t *g_parser = NULL;
...
static int PushFrameToRingbuf(RingBuffer_t *rb, const unsigned char *frameData, size_t frameSize)
{
    ...
    // 调试日志：记录第一个帧的长度字段
    static int first_size_pushed = 0;
    if (!first_size_pushed)
    {
        unsigned char *p = (unsigned char *)&frameSizeTmp;
        LOG_INFO("[AACParserThread] Pushing first frame size: frameSize=%zu, frameSizeTmp=%u, bytes=%02x %02x %02x %02x\n",
            frameSize, frameSizeTmp, p[0], p[1], p[2], p[3]);
        first_size_pushed = 1;
    }
    ...
    if (ret < 0)
    {
        LOG_ERR("[AACParserThread] Failed to push frame size to ringbuf\n");
        return -1;
    }
    ...
    if (ret < 0)
    {
        LOG_ERR("[AACParserThread] Failed to push frame data to ringbuf\n");
        return -1;
    }

    LOG_DEBUG("[AACParserThread] Frame pushed: size=%zu, ringbuf usage: %zu/%zu\n",
 ...
 * @brief 解析线程函数
 * 
 * 使用 aac_parser 解析码流文件，将帧推送到 ringbuf
 * 
...
static void* ParseThread(void *arg)
{
    ...
    int ret = prctl(PR_SET_NAME, "AACParser", 0, 0, 0);
    if (ret != 0)
    {
        LOG_WARN("[AACParserThread] Failed to set thread name\n");
    }

    LOG_INFO("[AACParserThread] Thread started, parsing AAC file: %s\n", params->input_file);

    // 创建 AAC 解析器
    g_parser = AACParserCreate(params->input_file, 0, params->loop_enabled);
    if (g_parser == NULL)
    {
        LOG_ERR("[AACParserThread] Failed to create AAC parser\n");
        g_running = 0;
        return NULL;
    }
    ...
    if (frameBuf == NULL)
    {
        LOG_ERR("[AACParserThread] Failed to allocate frame buffer\n");
        AACParserDestroy(g_parser);
        g_parser = NULL;
        g_running = 0;
        return NULL;
    }
    ...
    {
        // 获取下一帧
        int result = AACParserNextFrame(g_parser, &frame);
        if (result < 0)
        {
            LOG_ERR("[AACParserThread] Failed to get next frame\n");
            break;
        }
        else if (result > 0)
        {
            // EOF（仅在 loop_enabled=0 时）
            LOG_INFO("[AACParserThread] End of file reached, total frames: %d\n", frame_count);
            break;
        }
        ...
        {
            LOG_ERR("[AACParserThread] Frame too large: %zu bytes\n", frame.size);
            continue;
        }
        ...
        // 调试日志：记录第一个帧的数据（仅记录前10字节）
        static int first_frame_copied = 0;
        if (!first_frame_copied)
        {
            LOG_INFO("[AACParserThread] First frame copied: frameSize=%zu, frameBuf[0:10]=%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x, frame.data[0:10]=%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
                frame.size,
                frameBuf[0], frameBuf[1], frameBuf[2], frameBuf[3], frameBuf[4],
                frameBuf[5], frameBuf[6], frameBuf[7], frameBuf[8], frameBuf[9],
            ...
            first_frame_copied = 1;
        }
        
        // 推送到 ringbuf（使用拷贝后的数据）
        if (PushFrameToRingbuf(g_ringbuf, frameBuf, frame.size) < 0)
        {
            LOG_ERR("[AACParserThread] Failed to push frame to ringbuf, stopping\n");
            break;
        }
        ...
        {
            LOG_INFO("[AACParserThread] Parsed %d AAC frames (sample_rate_idx=%d, channels=%d)\n", 
                frame_count, frame.headerInfo.samplingFrequencyIndex, frame.headerInfo.channelConfig);
        }
    }
    ...
    LOG_INFO("[AACParserThread] Thread exited, total AAC frames parsed: %d\n", frame_count);
    return NULL;
}
...
int main(int argc, char *argv[])
{
    if (argc < 3)
    {
        ...
        printf("  input_file:    AAC stream file path (ADTS format)\n");
    ...
    LOG_INFO("[Main] Starting AAC file parser sample\n");
